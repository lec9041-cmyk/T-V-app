      ...
      <div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-sm flex items-center justify-center p-3 md:p-4">
        <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
          <div className="p-4 md:p-6 border-b border-gray-100 bg-white flex items-start justify-between gap-4">
            <div>
              <h2 className="text-xl md:text-2xl font-black text-gray-900">주관식 시험 결과</h2>
          ...
          <div className="flex-1 overflow-auto p-4 md:p-6 space-y-5">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div className="rounded-2xl bg-slate-50 border border-gray-200 p-4">
                ...
                <div className="text-2xl font-black text-gray-900 mt-1">{resultSummary.total}</div>
              </div>
              <div className="rounded-2xl bg-emerald-50 border border-emerald-100 p-4">
                <div className="text-xs font-bold text-emerald-600">정답 수</div>
                <div className="text-2xl font-black text-emerald-700 mt-1">{resultSummary.correct}</div>
              </div>
              <div className="rounded-2xl bg-rose-50 border border-rose-100 p-4">
                <div className="text-xs font-bold text-rose-600">오답 수</div>
                <div className="text-2xl font-black text-rose-700 mt-1">{resultSummary.wrong}</div>
              </div>
              <div className="rounded-2xl bg-blue-50 border border-blue-100 p-4">
                <div className="text-xs font-bold text-blue-600">정답률</div>
                <div className="text-2xl font-black text-blue-700 mt-1">{resultSummary.accuracy}%</div>
              </div>
              <div className="rounded-2xl bg-amber-50 border border-amber-100 p-4">
                <div className="text-xs font-bold text-amber-600">미입력 수</div>
                <div className="text-2xl font-black text-amber-700 mt-1">{emptyAnswerCount}</div>
              </div>
              <div className="rounded-2xl bg-indigo-50 border border-indigo-100 p-4">
                <div className="text-xs font-bold text-indigo-600">수동 수정</div>
                <div className="text-2xl font-black text-indigo-700 mt-1">{manualEditedCount}</div>
              </div>
            </div>
              ...
              {wrongResultItems.length === 0 ? (
                ...
                <div className="space-y-3">
                  {wrongResultItems.map((item) => (
                    <div key={`${item.word.day}-${item.word.no}-${item.word.index}`} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                      <div className="text-xs font-bold text-gray-400">DAY {item.word.day} · #{item.word.no}</div>
                      <div className="text-lg font-black text-gray-900 mt-1">{item.questionText}</div>
          ...
          <div className="border-t border-gray-100 bg-white p-4 md:p-5 flex flex-col sm:flex-row gap-2 sm:justify-end">
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-sm flex items-center justify-center p-3 md:p-4">
      <div className="w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
        <div className="p-4 md:p-6 border-b border-gray-100 bg-white flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl md:text-2xl font-black text-gray-900">주관식 시험</h2>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-6 space-y-4">
          {currentPageItems.map((item, offset) => {
            ...
            return (
              <div key={`${item.word.day}-${item.word.no}-${item.word.index}`} className="rounded-2xl bg-gradient-to-br from-white via-slate-50 to-indigo-50/30 border border-gray-200/70 shadow-sm p-4 md:p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xs font-bold text-gray-400">문제 {itemIndex + 1} · DAY {item.word.day} · #{item.word.no}</div>
                    <div className="text-xl md:text-2xl font-black text-gray-900 mt-2">{questionText}</div>
                    <div className="text-xs text-gray-500 mt-1">{direction === 'en2ko' ? '뜻을 입력하세요' : '영어 단어를 입력하세요'}</div>
                  </div>
                  {isGraded && (
                ...
                <div className="mt-4">
                  <input
                    ...
                    readOnly={isGraded}
                    placeholder="답 입력"
                    className="w-full rounded-xl border-2 border-gray-200 bg-white px-4 py-3 text-base font-semibold text-gray-900 outline-none transition-colors focus:border-blue-400 disabled:bg-gray-100 disabled:text-gray-700 disabled:cursor-not-allowed"
                  />
                </div>
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                      <div className="rounded-xl bg-white border border-gray-200 p-3">
                        <span className="font-bold text-gray-500">내가 입력한 답: </span>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button size="sm" variant={item.finalCorrect ? 'default' : 'outline'} onClick={() => setFinalCorrect(itemIndex, true)} className="rounded-xl">
                        <CheckCircle2 className="w-4 h-4 mr-1" />
                        정답 처리
                      </Button>
                      <Button size="sm" variant={item.finalCorrect === false ? 'destructive' : 'outline'} onClick={() => setFinalCorrect(itemIndex, false)} className="rounded-xl">
        ...
        <div className="border-t border-gray-100 bg-white p-4 md:p-5 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
